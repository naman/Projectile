...
class Batch(models.Model):
    ...
    pg_or_not = models.CharField(
        "Graduate or Post-Graduate", max_length=1, choices=QUALIFICATION, default='G')
...
class Job(models.Model):
    ...
    selectionprocess = models.CharField(
        "Selection Process", max_length=50, blank=True, null=True)
    ...
    job_stream = models.CharField(
        "Job Stream", max_length=3, choices=JOBSTREAMS, default='CSE')
    ...
    category = models.CharField(
        "Company Category", max_length=2, choices=CATEGORYCHOICES)
...
class Student(models.Model):
    ...
    gender = models.CharField("Gender", max_length=1,
                              choices=genderchoices, default='M')
    ...
    email_personal = models.EmailField(
        "Personal Email", max_length=70, blank=True, null=True)
    cgpa_ug = models.FloatField("Ug. CGPA", max_length=4, default=0)
    university_ug = models.CharField(
        "Undergraduate University", default="IIIT Delhi", max_length=100)
    ...
    percentage_twelfth = models.FloatField(
        "Class XII Aggregate Percent", max_length=4)

    cgpa_pg = models.FloatField(
        "Pg. CGPA", max_length=4, default=0, blank=True, null=True)

    university_pg = models.CharField(
        "Postgraduate University", max_length=100, blank=True, null=True)
    institution_pg = models.CharField(
        "Postgraduate College", max_length=100, blank=True, null=True)
    ...
    STATUS = (
        ('P', 'Placed'),
        ('N', 'Not Placed'),
        ('D', 'Debarred'),  # Debarred from dhcs
        # for students who live a thuglife :P a+ ppl will also be categorised
        # here
        ('NI', 'Not Interested')
    )
